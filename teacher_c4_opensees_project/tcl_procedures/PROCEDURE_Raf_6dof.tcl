proc modal { num_modes filename {eig_solver -genBandArpack}} {
	...
	set mode_data_single [lrepeat $num_modes [lrepeat 2 0.0]]
	...
	for {set imode 0} {$imode < $num_modes} {incr imode} {
		...
		lset mode_data_single $imode [list $frequency]
	...
	for {set i 0} {$i < $num_modes} {incr i} {
	    set tmp [list [expr $i+1.0] [lindex $mode_data_single $i] {*}[lindex $mode_MPM $i]]
		multiputs stdout $fp $tmp;
	}
