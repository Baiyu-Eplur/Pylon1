...
proc write_analysis_status {status message} {
	...
	if {[info exists opensees_output_dir]} {
		set status_path "$opensees_output_dir/analysis_status.txt"
	} else {
		set status_path "Output/analysis_status.txt"
	}
	set status_file [open $status_path w]
	puts $status_file "STATUS $status"
	puts $status_file "MESSAGE $message"
	if {[info exists STKO_VAR_time]} {puts $status_file "TIME $STKO_VAR_time"}
	if {[info exists total_duration]} {puts $status_file "TARGET_TIME $total_duration"}
	if {[info exists STKO_VAR_percentage]} {puts $status_file "PROGRESS [expr $STKO_VAR_percentage * 100.0]"}
	if {[info exists STKO_VAR_increment]} {puts $status_file "INCREMENT $STKO_VAR_increment"}
	if {[info exists STKO_VAR_analyze_done]} {puts $status_file "ANALYZE_RETURN_CODE $STKO_VAR_analyze_done"}
	if {[info exists factor]} {puts $status_file "FACTOR $factor"}
	if {[info exists min_factor]} {puts $status_file "MIN_FACTOR $min_factor"}
	if {[info exists event_stop_reason]} {puts $status_file "EVENT_REASON $event_stop_reason"}
	if {[info exists event_stop_max_disp]} {puts $status_file "EVENT_MAX_DISP $event_stop_max_disp"}
	if {[info exists event_stop_max_vel]} {puts $status_file "EVENT_MAX_VEL $event_stop_max_vel"}
	if {[info exists event_stop_max_accel]} {puts $status_file "EVENT_MAX_ACCEL $event_stop_max_accel"}
	if {[info exists event_stop_min_xi]} {puts $status_file "EVENT_MIN_XI $event_stop_min_xi"}
	if {[info exists event_stop_max_alpha]} {puts $status_file "EVENT_MAX_ALPHA $event_stop_max_alpha"}
	if {[info exists event_stop_clipped_fraction]} {puts $status_file "EVENT_CLIPPED_FRACTION $event_stop_clipped_fraction"}
	close $status_file
}
...
while 1 {
	...
	if {$STKO_VAR_analyze_done == 0} {
		...
		if {[expr $STKO_VAR_increment % 1] == 0} {
			if {[info exists opensees_output_dir]} {
